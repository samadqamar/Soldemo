...
namespace AdvancedPeopleSystem
{
    ...
    public class CharacterCustomization : MonoBehaviour
    {
        ...
        private void Start()
        {
          ...
          //  string test = "C:/Users/Samad/AppData/LocalLow/FourBstudio/MVP/apack_characters_data/MaleSettings\appack25_ali_1647380471.json";
            //test.Replace( "Samad", "ali" );
           // Debug.Log( "my replaced " + test );
        //    Debug.Log( "my test " + string.Format( string.Format( "Loaded {0}", saveDatas[saveDatas.Count - 1].path ).Replace(@"\","/" ) ));
        }
        private void Update()
        {
            AnimationTick();
        ...
        public void uploadFirebase()
        {
            ...
            storageReference = storage.GetReferenceFromUrl( "gs://mrcrash-f9d8a.appspot.com/Characters/"+PlayerPrefs.GetString("username"));
            var newMetadata = new MetadataChange();
            newMetadata.ContentType = ".json";

            StorageReference charcater = storageReference.Child( "cfg.json" );
            var saveDatas = GetSavedCharacterDatas();
         //   ApplySavedCharacterData( saveDatas[saveDatas.Count - 1] );
            
            string filelocation = string.Format( string.Format( "{0}", saveDatas[saveDatas.Count - 1].path ).Replace( @"\", "/" ) );
            Debug.Log( "File Location of uploads " + string.Format( string.Format( "{0}", saveDatas[saveDatas.Count - 1].path ).Replace( @"\", "/" ) ) );
            storageReference.PutFileAsync( filelocation,newMetadata, null ).ContinueWith( task =>
               {
                   if (task.IsCompleted)
                   {
        ...
        public void downloadfile()
        {
            ...
            string localfilelocation = "file://" + Application.streamingAssetsPath + "/Characters/"+ PlayerPrefs.GetString( "username" )+".json";
            storageReference.GetFileAsync( localfilelocation ).ContinueWith( task =>
               {
                   if (task.IsCompleted)
                   {
        ...
        {
          /*  var ext1 = Path.GetExtension( path );
            var data1 = File.ReadAllText( path );
            CharacterCustomizationSetup.CharacterFileSaveFormat format1 = CharacterCustomizationSetup.CharacterFileSaveFormat.Binary;
            format1 = CharacterCustomizationSetup.CharacterFileSaveFormat.Json;
            var setup1 = CharacterCustomizationSetup.Deserialize( data1, format1 );
            SetCharacterSetup( setup1 );*/
          //  Debug.Log( "location got" + path );
                var ext = Path.GetExtension(path);
                var data = File.ReadAllText(path);
           // Debug.Log( "data from path" + data );
           // Debug.Log( "location got 1 " + path );
            if (data.Length > 0)
                {
              //  Debug.Log( "location got 2 " + path );
                CharacterCustomizationSetup.CharacterFileSaveFormat format = CharacterCustomizationSetup.CharacterFileSaveFormat.Binary;
                    if (ext == ".json")
                    {
                  //  Debug.Log( "location got 3 " + path );
               ...
               // Debug.Log( "location got 4 " + path );
                var setup = CharacterCustomizationSetup.Deserialize(data, format);
               // Debug.Log( "location got 5 " + path );
                if (setup != null)
                    {
                  //  Debug.Log( "location got 6 " + path );
                    SetCharacterSetup(setup);
                  //  Debug.Log( "location got 7 " + path );
                    Debug.Log(string.Format("Loaded {0}", path));
                   //     Debug.Log( "Charcater loadedddddddddddddddd" + path );
                    }
                }
        ...
        public void SaveCharacterToFile(CharacterCustomizationSetup.CharacterFileSaveFormat format, string path = "", string name = "")
        {
            ...
            if (path.Length == 0)
            {
                ...
                string savedFileName = PlayerPrefs.GetString("username");
               // savepath = string.Format(  savedFileName, formatString );
            ...
            if (data.Length > 0)
            {
                ...
                {
                    File.Replace( savepath, savepath );
                }*/
